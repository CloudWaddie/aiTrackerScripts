...
def extract_js_strings():
    ...
    try:
        ...
        print("Checking for preferences")
        preferences_pattern = re.compile(r"preference:\s*['\"]([^'\"]+)['\"],?")
        preferences_match = preferences_pattern.findall(js_content)
        if preferences_match:
            print("\nFound preferences content:")
            for preference in preferences_match:
                print(preference)
        else:
            print("No preferences content found.")
